                        ...
                        <div class="cell medium-12" >
                            <div class="callout" data-equalizer-watch>
                                ...
                                <div class="text-center">
                                    ...
                                    <a class="button small alert" href="{{route('eliminar-causa-basica-hallazgo-inspeccion',['idInspeccion'=>$inspeccion->id,'tipoInspeccion'=>$inspeccion->medida,'idHallazgo'=>$idHallazgo,'idCausa'=>$causa->id])}}">Eliminar</a>
                                </div>
                            </div>
                        </div>
                    </div>
<!--**************REVEAL PARA MODIFICAR UNA CAUSA BASICA YA SELECCIONADA********************-->        
                    <div id='modificar-{{$causa->factor}}-{{$causa->categoria}}' class="reveal" data-reveal>
                        ...
                        <form name="frm-causas-basicas" method="post" action="{{route('crear-causas-basicas-hallazgo-inspeccion',['idInspeccion'=>$inspeccion->id,'tipoInspeccion'=>$inspeccion->medida,'idHallazgo'=>$idHallazgo])}}" class="frm-causas-basicas" id="frm-modificar-{{$causa->factor}}-{{$causa->categoria}}" data-factor='{{$causa->factor}}' data-categoria='{{$causa->categoria}}'>
                            {{ csrf_field() }}
                        ...
                        <!--Si no esta en el array de la base de datos se muestra la información de la categoria-->   
                            @if(!in_array($attrFactor."-".$idCategoria, $arrFactoresCategoriasCausasBD))
                                ...
                                <div id='{{$attrFactor}}-{{$idCategoria}}' class="reveal" data-reveal>
                                    ...
                                    <form name="frm-causas-basicas" method="post" action="{{route('crear-causas-basicas-hallazgo-inspeccion',['idInspeccion'=>$inspeccion->id,'tipoInspeccion'=>$inspeccion->medida,'idHallazgo'=>$idHallazgo])}}" class="frm-causas-basicas" id="frm-{{$attrFactor}}-{{$idCategoria}}" data-factor='{{$attrFactor}}' data-categoria='{{$idCategoria}}'>
                                        {{ csrf_field() }}
        ...
        <div id="div-btns-siguientePaso" class="row columns text-center">
            ...
            <a class="button small" href="{{ route('causas-inmediatas-hallazgo-inspeccion',['idInspeccion'=>$inspeccion->id,'tipoInspeccion'=>$inspeccion->medida,'idHallazgo'=>$idHallazgo]) }}">Anterior</a>

            <a class="button small alert" data-open="reveal-cancelar-crear">Cancelar</a>
            @if(count($causasBD)>0)
            <a class="button small success" href="{{ route('acciones-hallazgo-inspeccion',['idInspeccion'=>$inspeccion->id,'tipoInspeccion'=>$inspeccion->medida,'idHallazgo'=>$idHallazgo])}}">Siguiente</a>
            @endif
        </div>
    <script>
