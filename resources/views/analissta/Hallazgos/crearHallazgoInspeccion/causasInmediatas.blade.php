    ...
    <div class="row text-center">
                        ...
                        <div class="cell medium-12" >
                            <div class="callout" data-equalizer-watch>
                                ...
                                <div class="text-center">
                                    ...
                                    <a class="button small alert" href="{{route('eliminar-causa-inmediata-hallazgo-inspeccion',['idInspeccion'=>$inspeccion->id,'tipoInspeccion'=>$inspeccion->medida,'idHallazgo'=>$idHallazgo,'idCausa'=>$causa->id])}}">Eliminar</a>
                                </div>
                            </div>
                        </div>
                    </div>
        <!--**************REVEAL PARA MODIFICAR UNA CAUSA INMEDIATA YA SELECCIONADA********************-->        
                    <div id='modificar-{{$causa->factor}}-{{$causa->categoria}}' class="reveal" data-reveal>
                        ...
                        <form name="frm-causas-inmediatas" method="post" action="{{route('crear-causas-inmediatas-hallazgo-inspeccion',["idInspeccion"=>$inspeccion->id,"tipoInspeccion"=>$inspeccion->medida,"idHallazgo"=>$idHallazgo])}}" class="frm-causas-inmediatas" id="frm-modificar-{{$causa->factor}}-{{$causa->categoria}}" data-factor='{{$causa->factor}}' data-categoria='{{$causa->categoria}}'>
                            {{ csrf_field() }}
                        ...
                        <!--Si no esta en el array de la base de datos se muestra la información de la categoria-->   
                            @if(!in_array($attrFactor."-".$idCategoria, $arrFactoresCategoriasCausasBD))
                                ...
                                <div id='{{$attrFactor}}-{{$idCategoria}}' class="reveal" data-reveal>
                                    ...
                                    <form name="frm-causas-inmediatas" method="post" action="{{route('crear-causas-inmediatas-hallazgo-inspeccion',["idInspeccion"=>$inspeccion->id,"tipoInspeccion"=>$inspeccion->medida,"idHallazgo"=>$idHallazgo])}}" class="frm-causas-inmediatas" id="frm-{{$attrFactor}}-{{$idCategoria}}" data-factor='{{$attrFactor}}' data-categoria='{{$idCategoria}}'>
                                        {{ csrf_field() }}
        ...
        <div id="div-btns-siguientePaso" class="row columns text-center">
            ...
            @if($inspeccion->medida == 'obligatoria' || $inspeccion->medida == 'sugerida')
                <a class="button small" href="{{ route('peligro-asociado-hallazgo-inspeccion',["idInspeccion"=>$inspeccion->id,"tipoInspeccion"=>$inspeccion->medida,"idHallazgo"=>$idHallazgo]) }}">Anterior</a>
            @else
                <a class="button small" href="{{ route('crear-hallazgo-inspeccion',["idInspeccion"=>$inspeccion->id,"tipoInspeccion"=>$inspeccion->medida,"idHallazgo"=>$idHallazgo]) }}">Anterior</a>
            @endif
            <a class="button small alert" data-open="reveal-cancelar-crear">Cancelar</a>
            @if(count($causasBD)>0)
                <a class="button small success" href="{{ route('causas-basicas-hallazgo-inspeccion',["idInspeccion"=>$inspeccion->id,"tipoInspeccion"=>$inspeccion->medida,"idHallazgo"=>$idHallazgo])}}">Siguiente</a>
            @endif
