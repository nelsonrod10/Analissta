...
<?php
    ...
    if(isset($idHallazgo)){
        ...
        $procesoId = $hallazgoBD->proceso_id;
    }elseif(isset($inspeccion->peligro)){
        $procesoId = $inspeccion->peligro->actividad->proceso_id;
    }
?>
@section('content')
    ...
    <div class="row">
        <div class="columns small-12 medium-8 small-centered">
            <form method="post" name="datosGeneralesHallazgo" action="{{route('crear-datos-generales-hallazgo-inspeccion',["idInspeccion"=>$inspeccion->id,"tipoInspeccion"=>$inspeccion->medida,"idHallazgo"=>$idHallazgo])}}">
                {{ csrf_field() }}
                        ...
                        <div class="columns small-12 medium-6 end">
                            <select name="proceso" required="true">
                                ...
                                @foreach ($sistema->procesos as $proceso)
                                    <option value="{{$proceso->id}}"  <?php echo ($procesoId == $proceso->id)?"selected":""?> >
                                        {{$proceso->nombre}}
                                    </option>
                                @endforeach
                            </select>
