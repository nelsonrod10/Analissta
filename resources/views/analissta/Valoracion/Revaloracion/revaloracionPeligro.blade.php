...
<?php
    ...
    if($peligro->cortoPlazo){
        $ndCPbd = $peligro->cortoPlazo->nd;
        $neCPbd = $peligro->cortoPlazo->ne;
        $ncCPbd = $peligro->cortoPlazo->nc;
        $npCPbd = $peligro->cortoPlazo->np;
        $nriCPbd = $peligro->cortoPlazo->nri;      
    }
    
    if($peligro->largoPlazo){
        $ndLPbd = $peligro->largoPlazo->nd;
        $neLPbd = $peligro->largoPlazo->ne;
        $ncLPbd = $peligro->largoPlazo->nc;
        $npLPbd = $peligro->largoPlazo->np;
        $nriLPbd = $peligro->largoPlazo->nri;      
    }
?>
...
<div class="row">
    <div class="columns small-12 medium-8 small-centered">
        <form id="frm-nuevoPeligroValoracion" name="frm-nuevoPeligroValoracion" method="POST" action="{{route('guardar-revaloracion-peligro',['idPeligro'=>$peligro])}}">
            {{ csrf_field() }}  
            
            <div class="row columns text-center" >
                <div style="text-decoration: underline"><b>REVALORACION PELIGRO </b></div>
                <div>{{ $clasificacionPeligro }} - {{ $categoriaPeligro }}</div>
            </div>
            <div class="row columns text-center" style="text-decoration: underline">
                <b>REVALORACION DEL RIESGO</b>
            </div>
            ...
            <div class="row">
                <div class="columns small-8 small-centered">
                    <div class="callout alert text-center text-white">
                        <i>Esta haciendo una revaloración, si no desea hacer ningún cambio en este paso haga click en "Siguiente"</i>
                    </div>
                </div>
