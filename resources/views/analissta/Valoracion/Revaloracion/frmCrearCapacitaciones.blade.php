            ...
            @foreach($capacitacionesValoracion as $capacitacionValoracion)
                <div class="row">
                    ...
                    <div class="columns small-3 end">
                        <form name="frm-eliminar-capacitacion" method="post" action="{{ route('eliminar-medida-intervencion-revaloracion',['idPeligro'=>$peligro])}}">
                            {{ csrf_field() }}  
        ...
        <div class="columns small-12 small-centered">
            @if(count($capacitacionesDisponibles)>0)
                @foreach($capacitacionesDisponibles as $capacitacionDisponible)
                    @php
                        $capacitacionProgramada = CapacitacionesValoracione::where('peligro_id',$peligro->id)
                    ...
                    <div class="row">
                        ...
                        <div class="columns small-5 text-left">
                            ...
                            @else
                                <form name="frm-eliminar-capacitacion" method="post" action="{{ route('crear-medida-intervencion-revaloracion',['idPeligro'=>$peligro]) }}">
                                    {{ csrf_field() }}  
...
<div class="reveal" data-reveal="" id="div-reveal-newCapacitacion">
    ...
    <form id="frm-nuevoPeligroMedidas" name="frm-nuevoPeligroMedidas" method="POST" action="{{ route('crear-medida-intervencion-revaloracion',['idPeligro'=>$peligro]) }}">
        {{ csrf_field() }}  
