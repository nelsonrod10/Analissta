...
?>
<div id="modal-programar-medica-ocupacional-{{$evaluacion->id}}" class="reveal" data-reveal>
    <div class="row columns text-center">
        <br/>
        <div class="row columns text-center">
            <h5>Programar Evaluación Médica Ocupacional para 
                {{ucwords($evaluacion->empleado->nombre)}} {{ucwords($evaluacion->empleado->apellidos)}}
            </h5>
        </div>
        <br/>
        <form action="{{route('programar-evaluacion-medica.update',$evaluacion)}}" enctype="multipart/form-data" method="post">
            {{ csrf_field() }}
            {{ method_field('PUT')}}
            <div class="row ">
                <div class="columns small-12">
                    <label for="fecha-programada">Fecha de Programación de la evaluación</label>
                </div>
            </div>    
            <div class="row ">    
            ...
            <div class="row columns text-center">
                ...
                <button type="submit" class="button small">Programar</button>
            </div>    
        </form>
