...
use App\PlanesGestion\PveGeneralLineaBase;
use App\PlanesGestion\PveGeneralMeta;

class PveGeneralController extends Controller
{
    public function guardarData(){
        ...
        PveGenerale::where('id',$data['idPve'])
    ...
    private function guardarMetasIncidencia($idPve,$objetivo,$meta,$unidad){
        $existeIncidencia = PveGeneralMeta::where('pve_generales_id',$idPve)
        ...
        PveGeneralMeta::create([
           "nombreMeta" => 'Eficacia Incidencia',
           "pve_generales_id" => $idPve, 
           "objetivo" => $objetivo,
           "unidad"   => $unidad,
           "valorMeta"     => $meta, 
        ]);
    ...
    private function guardarMetasPrevalencia($idPve,$objetivo,$meta,$unidad){
        $existePrevalencia = PveGeneralMeta::where('pve_generales_id',$idPve)
    ...
    private function guardarMetasMortalidad($idPve,$objetivo,$meta,$unidad){
        $existeCasosNuevos = PveGeneralMeta::where('pve_generales_id',$idPve)
    ...
    private function lineaBaseIncidencia($idPve,$arrIncidencia){
        $existeBaseIncidencia = PveGeneralLineaBase::where('pve_generales_id',$idPve)
                ->where('nombreMeta','Eficacia Incidencia')
                ->get();
        ...
        if(count($existeBaseIncidencia)>0){
            ...
            $baseIncidencia = PveGeneralLineaBase::create([
                'pve_generales_id' => $idPve,
                'nombreMeta'      => 'Eficacia Incidencia',
             ]);
        }
    ...
    private function lineaBasePrevalencia($idPve,$arrPrevalencia){
        $existeBasePrevalencia = PveGeneralLineaBase::where('pve_generales_id',$idPve)
                ->where('nombreMeta','Eficacia Prevalencia')
                ->get();
        ...
        if(count($existeBasePrevalencia)>0){
            ...
            $basePrevalencia = PveGeneralLineaBase::create([
                'pve_generales_id' => $idPve,
                'nombreMeta'      => 'Eficacia Prevalencia',
             ]);
        }
    ...
    private function lineaBaseMortalidad($idPve,$arrMortalidad){
        $existeBaseCasosNuevos = PveGeneralLineaBase::where('pve_generales_id',$idPve)
                ->where('nombreMeta','Eficacia Casos Nuevos')
                ->get();
        ...
        if(count($existeBaseCasosNuevos)>0){
            ...
            $baseCasosNuevos = PveGeneralLineaBase::create([
                'pve_generales_id' => $idPve,
                'nombreMeta'      => 'Eficacia Casos Nuevos',
             ]);
        }
